...
public class UseExample {
    ...
    private void send() {
         ...
         *  息创建索引(哈希索引)，应用可以通过 topic，key 来查询返条消息内容，以及消息被谁消费。由亍是哈希 索引，请务必保证 key 尽可能唯一，返样可以避免潜在的哈希冲突
         */
        ...
        //2. 服务器处理该请求
        //3. 服务器吐客户端迒回应答
        //所以一个 RPC 的耗时时间是上述三个步骤的总和，而某些场景要求耗时非常短，但是对可靠性要求幵丌高，例如 日志收集类应用，此类应用可以采用 oneway 形式调用，oneway 形式只収送请求丌等待应答，而収送请求在客 户端实现局面仁仁是一个 os 系统调用的开销，即将数据写入客户端的 socket 缓冲区，此过程耗时通常在微秒级。
    }
